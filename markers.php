...
$query0 = "SELECT r.username, r.latitude, r.longitude, GROUP_CONCAT(CONCAT('<li>',ri.category, ': ', ri.item, ' (', ri.quantity, ')','</li>') SEPARATOR '') AS items
          FROM rescuers r
...
$query1 = "SELECT r.username, r.latitude, r.longitude, GROUP_CONCAT(CONCAT('<li>', ri.category, ': ', ri.item, ' (', ri.quantity, ')', '</li>') SEPARATOR '') AS items
          FROM rescuers r
          JOIN rescuer_inventory ri ON r.username = ri.username
...
while ($row = mysqli_fetch_assoc($result1)) {
    ...
    // Create a marker in the correct format
    $marker_rescuer_request = "L.marker([$latitude, $longitude]).bindPopup('Username: <b>$username</b><br>Items in Car:<ul>$items</ul>')";
